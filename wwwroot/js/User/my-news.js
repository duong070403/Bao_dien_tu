document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.btn').forEach(button => {
        button.addEventListener('click', function (e) {
            if (this.contains(e.target)) {  // Only create ripple if clicked directly on button
        ...
        fetch('/News/Archive/' + id, {
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    return response.json().then(data => Promise.reject(data.message || 'Lưu trữ thất bại'));
                }
            })
            .catch(error => showToast('Có lỗi xảy ra khi lưu trữ: ' + error, 'danger'));
    };
}
...
function showRepostModal(newsId) {
    ...
    // Set up confirm button handler
    document.getElementById('confirmRepostButton').onclick = function () {
        ...
        fetch('/News/Repost/' + id, {
            ...
            .catch(error => showToast('Có lỗi xảy ra khi đăng lại tin tức: ' + error, 'danger'));
    };
}
